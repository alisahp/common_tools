resource "kubernetes_service_account" "mybestsea_service_account" {
  depends_on = [
  ...
  metadata {
    name = "mybestsea-service-account"
    namespace = "${var.deployment_environment}"
  }
  secret {
    name = "${kubernetes_secret.mybestsea_service_account_secret.metadata.0.name}"
  }
  automount_service_account_token = true
}

resource "kubernetes_secret" "mybestsea_service_account_secret" {
  depends_on = [
    "kubernetes_namespace.service_tools",
    "kubernetes_service_account.tiller",
    "kubernetes_secret.tiller"
